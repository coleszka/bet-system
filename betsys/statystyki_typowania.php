...
<?php
			...
			$query_stat = $conn->query("SELECT COUNT(id_user) FROM postawione_kupony WHERE id_user='{$_SESSION['id']}'");  /*WHERE date='$today'*/
			...
			$l_w_k=0;
			$l_p_k=0;
			$query_moje_kupony = $conn->query("SELECT id_user, numer_kuponu, wygrana, wynik  FROM postawione_kupony WHERE postawione_kupony.id_user='{$_SESSION['id']}'"); 
			$rows = $query_moje_kupony->num_rows;
				...
				echo "<br/>Ilość wygranych kuponów: <b>".number_format(($l_w_k/$l_zwer_kup)*100, 2)."%</b>";

				echo "<br/>Ilość przegranych kuponów: <b>".number_format(($l_p_k/$l_zwer_kup)*100, 2)."%</b>";
